...
campaign_ad_sets=ad_campaigns.merge(ad_sets, left_on='id', right_on='campaign_id')

#Merge campaign_ad_sets with ads
campaign_ad_sets_ads=campaign_ad_sets.merge(ads, left_on='id_y', right_on='adset_id')

#Merge campaign_ad_sets_ads with insights
campaign_insights=campaign_ad_sets_ads.merge(insights, left_on='id', right_on='ad_id')
...
#26. Number Facebook Ad Driven Allocated Starters
aquisition_source_idea=applicant_features_joiners[['state','utm_source','solution_id_x_y']]
Facebook_Mapping = {'':'Organic','intercom': 'Intercom','FacebookPaid_GIY': 'FacebookAds','FacebookPaid_IdeaFund': 'FacebookAds', 'FacebookPaid_Poetry': 'FacebookAds','FacebookPaid_MNFund': 'FacebookAds','FacebookPaid_WelcomingWeek': 'FacebookAds','FacebookPaid_StreetFeast': 'FacebookAds','FacebookPaid_Kaboom': 'FacebookAds','FacebookPaid_MensShed': 'FacebookAds'}
aquisition_source_idea['utm_source']=aquisition_source_idea['utm_source'].map(Facebook_Mapping)
aquisition_source_idea = aquisition_source_idea[aquisition_source_idea['state'] != 'active']
facebook_driven_starters=aquisition_source_idea[aquisition_source_idea['utm_source']=='FacebookAds']
count_facebook_driven_starters=facebook_driven_starters['solution_id_x_y'].count()

#27. Average Cost Per Approved Starter
cost_per_approved_starter= total_fund_spend/ count_facebook_driven_starters

#28. Number Facebook Ad Driven Active Starters Per Idea
state_mapping = {'approved':'paid', 'rejected':'failed', 'failed':'failed', 'paid':'paid'}
aquisition_source_idea['state']=aquisition_source_idea['state'].map(state_mapping)
aquisition_source_idea_paid=aquisition_source_idea[aquisition_source_idea['state']=='paid']
aquisition_source_idea_paid_fb = aquisition_source_idea_paid[aquisition_source_idea_paid['utm_source']=='FacebookAds']
count_facebook_driven_active_starters= aquisition_source_idea_paid_fb['utm_source'].count()
count_facebook_driven_active_starters

#29. Average Cost per Active Starter
cost_per_active_stater = total_fund_spend / count_facebook_driven_active_starters

#30. Cost Per Approved Starter Per Idea
...
def budget_remaining_callback(fund_namee):
        ...
        #Total Pack Costs
        unit_pack_cost= 16.70 * fund_namee
        total_pack_cost = number_received_pack * unit_pack_cost
        #Total Staff Costs
        staff_daily_rate= 4000.0/22
        total_staff_costs = staff_daily_rate * 88
        #Total Delivery costs
        total_delivery_cost = total_staff_costs + total_pack_cost + total_fund_spend

        budget_remaining=fund_budget-total_delivery_cost

        budget_remaining_money=Money(budget_remaining,'USD')
        budget_remaining_eur=budget_remaining_money.format('en_US')
...
def delivery_cost_indicator_callback(fund_slug_value):
        ...
        total_fund_spend = fund_ads['spend'].sum()
        #Total Pack Costs
        unit_pack_cost= 16.70 * fund_slug_value
        total_pack_cost = number_received_pack * unit_pack_cost
        #Total Staff Costs
        staff_daily_rate= 4000.0/22
        total_staff_costs = staff_daily_rate * 88
        ...
        total_delivery_cost_money=Money(total_delivery_cost,'USD')
        total_delivery_cost_eur=total_delivery_cost_money.format('en_US')
...
def total_active_callback(fund_slug_value):
    filtered_fund_accounts=accounts.loc[accounts['parent_account_id'] == fund_slug_value]
    list_of_accounts=filtered_fund_accounts['id'].tolist()
    filtered_fund_paid_accounts=accounts[accounts['parent_account_id'].isin(list_of_accounts)]
    number_paid_accounts=filtered_fund_paid_accounts.shape[0]
...
def total_ad_spend_callback(fund_namee):
    ...
    fund_ads=campaign_insights[campaign_insights['name_x'].str.contains(slug)]
    total_fund_spend = fund_ads['spend'].sum()
    total_ad_spend_cash=Money(total_fund_spend,'USD')
    total_ad_spend_eur_amount=total_ad_spend_cash.format('en_US')
...
def ad_cost_approved_callback(fund_namee):
    ...
    #27. Average Cost Per Approved Starter
    cost_per_approved_starter= total_fund_spend/ count_facebook_driven_starters
    cost_per_approved_starter_cash=Money(cost_per_approved_starter,'USD')
    cost_per_approved_starter_eur_amount=cost_per_approved_starter_cash.format('en_US')
...
def ad_cost_active_callback(fund_namee):
    ...
    #27. Average Cost Per Approved Starter
    cost_per_active_starter= total_fund_spend/count_facebook_driven_active_starters
    cost_per_active_starter_cash=Money(cost_per_active_starter,'USD')
    cost_per_active_starter_eur_amount=cost_per_active_starter_cash.format('en_US')
...
def wait_on_pack_callback(fund_namee):
    filtered_applicant_features_joiners=applicant_features_joiners[applicant_features_joiners['parent_account_id'] == fund_namee]
    non_rejected_applicants=filtered_applicant_features_joiners[filtered_applicant_features_joiners['state']!='rejected']
    want_starter_pack=non_rejected_applicants[non_rejected_applicants['receive_starter_pack']==True]
    number_want_pack=want_starter_pack['receive_starter_pack'].count()
    starter_pack_delivered=non_rejected_applicants[non_rejected_applicants['starter_pack_sent_at'] != 'NaT']
    number_received_pack=starter_pack_delivered['starter_pack_sent_at'].count()
...
def wait_on_call_callback(fund_namee):
    filtered_applicant_features_joiners=applicant_features_joiners[applicant_features_joiners['parent_account_id'] == fund_namee]
    non_rejected_applicants=filtered_applicant_features_joiners[filtered_applicant_features_joiners['state']!='rejected']
    starters_called=non_rejected_applicants[non_rejected_applicants['received_starter_call_at']!='NaT']
    number_starters_called=starters_called['received_starter_call_at'].count()
    total_number_applicants=applicant_features_joiners['id_x'].count()
    number_waiting_call=total_number_applicants-number_starters_called
...
def pack_wait_time_callback(fund_namee):
    ...
    pack_wait_time=want_starter_pack['starter_pack_sent_at'] - want_starter_pack['created_at_y_x']
    pack_wait_time=pack_wait_time.mean()
    pack_wait_days=pack_wait_time.days
...
def fund_left_callback(fund_namee):
    ...
    filtered_fund_paid_accounts=accounts[accounts['id'].isin(list_of_accounts)]
    total_paid_out=filtered_fund_paid_accounts['balance_cents'].sum()/100
    total_fund_remaining = fund_budget - total_paid_out

    total_fund_remaining_cash=Money(total_fund_remaining,'USD')
    total_fund_remaining_eur_amount=total_fund_remaining_cash.format('en_US')
...
def fund_paid_out_callback(fund_namee):
    ...
    return total_paid_out_eur_amount
...
def funnel_callback(fund_account):
    return active_funnel_chart(fund_account)
...
def acquisition_callback(fund_account_value):
    return acquisition_chart(fund_account_value)
...
def idea_demand_table_callback(fund_namee):
    return idea_demand_table(fund_namee)
